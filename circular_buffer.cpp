/*
Circular Buffer - It's a FIFO algorithm, it allows asynchronous write and read operations.

Data is stored in a continous loop, in a constant length buffer that has two pointers that represents beggining and end as HEAD and TAIL.
Oldest data can be replaced by new coming data. 
Each time we add data, write (HEAD) pointer moves forward.
Each time we read data, read (TAIL) pointer moves forward.
If write pointer is right behind read pointer, read pointer is moved forward each time write pointer is behind,
and .
If write pointer has walked arround the whole buffer, and caught up a write pointer, then write pointer is moved forward,
and data it was pointing at is overwritten by HEAD (it's oldest HEAD data that was written and first TAIL data to read)
...
{
    memset(m_buffer, 0, m_buffer_size * sizeof(*m_buffer));
    m_head = 0;
    m_tail = 0;
    m_size = 0;
    m_isFull = false;
